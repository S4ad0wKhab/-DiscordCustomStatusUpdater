# ​บอทอัปเดตสถานะ Discord อัตโนมัติ (นับถอยหลังเหตุการณ์แบบวัน)
**⚠️⚠️คำแนะนำให้เลือนลงไปอ่านในส่วนของคำเตือนก่อนลงมือใช้โค้ด/คำสั่งเพื่อตัวคุณเอง⚠️⚠️**
**คลิปสอนทั้งหมด:** [tiktok](https://vt.tiktok.com/ZSQFGHqPC/)
...
```
**คำสั่งใน termux ย้ายที่อยู่ไฟล์ (ที่อยู่ไฟล์ตามที่เราสอนทำในคลิปนะครับ ถ้าไม่ได้อยู่ที่เดียวกันลองถามเอไอดู):**
```
cd /storage/emulated/0/bot/birthday_bot
```
**ติดตั้งแพ็คเกจใน termux หรืออะไรก็ได้:**
```
pkg install python -y && pip install requests
```
**คุณสามารถแก้ไขข้อความที่บอทจะตั้งสถานะได้! ในส่วน:**
```
def get_status_text():
    today = date.today()
    if today == BIRTHDAY:
...
```
**แก้ไขโทเค็นบัญชีดิสคอร์ด**
```
TOKER = "YOUR_TOKEN"
# ให้ลบ YOUR_TOKEN แล้วเปลี่ยนเป็นโทเค็นบัญชีดิสคอร์ดของคุณ
```
**แก้ไขวัน**
```
BIRTHDAY = date(2026, 11, 12)
# 11=เดือน 12=วัน
```
# ข้อควรระวัง⚠️
**ห้ามแสดงโทเค็นบัญชีดิสคอร์ดของคุณแก่คนอื่น** เพราะคุณอาจถูกคนอื่นควบคุมบัญชีได้!

บอทตัวนี้เป็นบอทประเภท Self-bot ซึ่งผิดกฎร้ายแรงของ Discord แนะนำอย่าแก้ไขโค้ดนี้เด็ดขาดยกเว้นส่วนที่ไม่ใช่เวลา หรือหากจำเป็นจริงๆควรถามเอไอแต่ต้องละเอียดนิดหน่อย เช่น ช่วยแก้โค้ดนี้<โค้ด> สิ่งที่อยากจะแก้~~~~~ ทำให้ไม่โดนแบนบัญชีดิสคอร์ด
# ใบอนุญาต Apache License 2.0
[Apache License 2.0](https://github.com/S4ad0wKhab/-DiscordCustomStatusUpdater/blob/main/LICENSE) (เมื่อ 6 มิ.ย. 2026)
